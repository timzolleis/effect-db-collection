...
import { Cause, Effect, Fiber, Layer, ManagedRuntime, Option, Ref } from 'effect'
...
interface LocalEffectCollectionConfig<TItem extends object, TRuntimeContext> {
  ...
  onInsert?: (
    params: InsertMutationFnParams<TItem>
  ) => Effect.Effect<TItem[] | RefetchResponse, unknown, TRuntimeContext>

  /** Handler for update mutations - should return the new state after update */
  onUpdate?: (
    params: UpdateMutationFnParams<TItem>
  ) => Effect.Effect<Array<TItem> | RefetchResponse, unknown, TRuntimeContext>
...
export function localEffectCollectionOptions<TItem extends object, TRuntimeContext>(
  ...
  const sync: SyncConfig<TItem>['sync'] = (params) => {
    ...
    const captureServiceLayer = Ref.set(
      collectionServiceRef,
      Option.some(CollectionService.Default({ collectionId: config.id, syncParams: params }))
    )
    ...
    const fiber = config.runtime.runFork(
      initialSyncHandler.pipe(
        Effect.provide(
    ...
    return () => {
      return Effect.gen(function* () {
        yield* Effect.all([Ref.set(collectionServiceRef, Option.none())])
  ...
  const sendEffectTransaction = async (
    ...
    return sendTransaction({
      ...
      Effect.tapErrorCause((failure) => {
        if (Cause.isFailType(failure)) {
          return Effect.logError(`[sendTransaction]: Error during transaction`, { error: failure.error })
        } else {
          return Effect.logError(`[sendTransaction]: Unexpected error during transaction`, {
            error: Cause.pretty(failure)
          })
        }
      }),
      Effect.map(() => void 0),
