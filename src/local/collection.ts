...
interface LocalEffectCollectionConfig<TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>> {
  ...
  ) => Effect.Effect<TItem[] | RefetchResponse, unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>

  /** Handler for update mutations - should return the new state after update */
  onUpdate?: (
    params: UpdateMutationFnParams<TItem>
  ) => Effect.Effect<Array<TItem> | RefetchResponse, unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>

  /** Handler for delete mutations - returns void as items are removed from state */
  onDelete?: (
    params: DeleteMutationFnParams<TItem>
  ) => Effect.Effect<void | RefetchResponse, unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>
 ...
 */
export function localEffectCollectionOptions<
  TItem extends object,
  TRuntime extends ManagedRuntime.ManagedRuntime<any, any>
>(config: LocalEffectCollectionConfig<TItem, TRuntime>): CollectionConfig<TItem> {
  ...
  const sendEffectTransaction = async (
    ...
    effectHandler?: (
      params: any
    ) => Effect.Effect<Array<TItem> | void | RefetchResponse, unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>,
