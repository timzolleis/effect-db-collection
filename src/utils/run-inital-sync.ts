...
import {
    ...
    SyncDataResponseError, WriteCollectionItemError
} from "../local/errors";
...
const runEffectQuery = <TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>>({effect, collectionId}: {effect: Effect.Effect<TItem[], unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>, collectionId: string}) => Effect.gen(function* () {
    ...
    yield* Effect.logInfo(`[${collectionId}] Query completed`, { itemCount: data.length })
    return data
}).pipe(
...
const clearCollection = <TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any ,any>>({refs, collectionId}: {refs: Refs<TItem>, collectionId: string}) => Effect.gen(function* () {
...
export const runInitialSync = <TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>>({config, refs}: RunInitialSyncOptions<TItem, TRuntime>) => Effect.gen(function* () {
    const syncParams = yield* Ref.get(refs.syncParams)
    if (Option.isNone(syncParams)) {
        return yield* new CollectionNotInitializedError({
            operation: "delete",
