...
export const rollbackTransaction = <TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>>({
  mutationType,
  config,
  params,
  refs
}: RollbackTransactionOptions<TItem, TRuntime>) =>
  Effect.gen(function* () {
    ...
    Effect.catchAll((error) =>
      Effect.logError(`[rollbackTransaction]: failed to rollback transaction`, {
        error
      })
    )
  )
