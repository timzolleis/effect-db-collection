...
interface SendTransactionOptions<TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>> {
  ...
  effect: (
    params: any
  ) => Effect.Effect<Array<TItem> | RefetchResponse | void, unknown, ManagedRuntime.ManagedRuntime.Context<TRuntime>>
  mutationType: 'insert' | 'update' | 'delete'
  refs: Refs<TItem>
}

export const sendTransaction = <TItem extends object, TRuntime extends ManagedRuntime.ManagedRuntime<any, any>>({
  effect,
  params,
  config,
  refs,
  mutationType
}: SendTransactionOptions<TItem, TRuntime>) =>
  Effect.gen(function* () {
    ...
    if (!Array.isArray(serverResponse)) {
      ...
      return yield* runInitialSync({
        ...
        Effect.catchAll((error) =>
          Effect.logDebug(`[sendTransaction]: failed to run initial sync to refetch`, { error })
        )
      )
    }
